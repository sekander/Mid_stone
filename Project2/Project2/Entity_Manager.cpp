...
namespace Sekander
{
	...
	void EntityManager::newEdge(std::string name, float xo, float yo, float xd, float yd)
	{
		...
		entity->CreateEdge(xo, yo, xd, yd);
		this->entities.insert(std::make_pair("EDGE:" + name, entity));


		vertices[0] = sf::Vertex(sf::Vector2f(xo + 15, yo*-1 + 16), sf::Color::Cyan);
		vertices[1] = sf::Vertex(sf::Vector2f(xd + 15, yd*-1 + 16), sf::Color::Cyan);
		...
		std::cout << "VERTices size " << vertices_vec.size() << std::endl;
