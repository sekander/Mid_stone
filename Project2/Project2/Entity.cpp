...
namespace Sekander
{
		...
		unsigned const short int y_frames)
	{
		...
		if (x_frames == 0 || y_frames == 0)
		{
			...
			this->shape->SetAsBox(this->texture->getSize().x * 0.5f,
								  this->texture->getSize().y * 0.5f);
	...
	void Entity::CreateEdge(float x_point_origin, float y_point_origin, 
						    float x_point_dest, float y_point_dest)
	{
		this->bodyDef = new b2BodyDef();
		this->bodyDef->type = b2_dynamicBody;
		...
		this->fixtureDef->shape = edge;

		this->edge->SetTwoSided(b2Vec2(x_point_origin, y_point_origin), 
