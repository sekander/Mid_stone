...
namespace Sekander
{
	...
	public:
		...
		void Update(float dt)
		{
			...
			//gun_clock.getElapsedTime().asMilliseconds();
			//capture_time += _clock.getElapsedTime().asSeconds();
			...
			for (auto& entity : entities)
			{
				...
				if (!entity.first.find("EDGE:floor"))
				{
					...
					for (b2ContactEdge* ce = this->_data->manager.Get(entity.first)->body->GetContactList(); ce;
						ce = ce->next)
					{
